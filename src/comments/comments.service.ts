...
export const createcommentService = async (User: TIComment):Promise <string> => {
    await db.insert(comment).values(User)
    return "comment created successfully";
}

export const updatecommentService = async (id: number, userData: TIComment): Promise <string> => {
